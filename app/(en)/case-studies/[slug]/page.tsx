...
export function generateStaticParams() {
  return caseStudiesByLang.en.map(({ slug }) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const ref = caseStudyRef(params.slug, 'en');
  if (!ref) return {};
  const doc = await readDoc(`${ref.file}.md`);
  return docMeta('en', `/case-studies/${params.slug}/`, doc.title, doc.perex);
}

export default function Page({ params }: Props) {
  return <CasePage lang="en" slug={params.slug} />;
}
